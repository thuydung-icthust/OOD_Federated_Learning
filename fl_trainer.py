...
def calc_norm_diff(gs_model, vanilla_model, epoch, fl_round, mode="bad"):
    ...
    for p_index, p in enumerate(gs_model.parameters()):
        norm_diff += (
            torch.norm(
                list(gs_model.parameters())[p_index]
                - list(vanilla_model.parameters())[p_index]
            )
            ** 2
        )
    ...
    if mode == "bad":
        # pdb.set_trace()
        logger.info(
            "===> ND `|w_bad-w_g|` in local epoch: {} | FL round: {} |, is {}".format(
                epoch, fl_round, norm_diff
            )
        )
    elif mode == "normal":
        logger.info(
            "===> ND `|w_normal-w_g|` in local epoch: {} | FL round: {} |, is {}".format(
                epoch, fl_round, norm_diff
            )
        )
    elif mode == "avg":
        logger.info(
            "===> ND `|w_avg-w_g|` in local epoch: {} | FL round: {} |, is {}".format(
                epoch, fl_round, norm_diff
            )
        )
...
def fed_avg_aggregator(net_list, net_freq, device, model="lenet"):
    ...
    for p_index, p in enumerate(net_list[0].parameters()):
        ...
        for net_index, net in enumerate(net_list):
            ...
            params_aggregator = (
                params_aggregator
                + net_freq[net_index] * list(net.parameters())[p_index].data
            )
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if not pgd_attack:
            ...
            if proj == "l_inf":
                ...
                if (
                    batch_idx % project_frequency == 0
                    or batch_idx == len(train_loader) - 1
                ) and (torch.norm(w_vec - model_original_vec) > eps):
                    # project back into norm ball
                    w_proj_vec = (
                        eps
                        * (w_vec - model_original_vec)
                        / torch.norm(w_vec - model_original_vec)
                        + model_original_vec
                    )
...
class FrequencyFederatedLearningTrainer(FederatedLearningTrainer):
    def __init__(self, arguments=None, *args, **kwargs):
        ...
        self.adversarial_local_training_period = arguments[
        ...
        if self.poison_type == "ardis":
            ...
            self.ardis_dataset = datasets.get_southwest_dataset(
        ...
        #     self._defender = None
            ...
            arguments["defense_technique"] == "norm-clipping"
            or arguments["defense_technique"] == "norm-clipping-adaptive"
        ):
            self._defender = WeightDiffClippingDefense(
                norm_bound=arguments["norm_bound"]
            )
            ...
            self._defender = Krum(
                mode="krum", num_workers=self.part_nets_per_round, num_adv=1
            )
        elif arguments["defense_technique"] == "multi-krum":
            self._defender = Krum(
                mode="multi-krum", num_workers=self.part_nets_per_round, num_adv=1
            )
            ...
            self._defender = KrMLRFL(
                num_workers=self.part_nets_per_round, num_adv=1, num_valid=1
            )
        ...
        elif arguments["defense_technique"] == "ddpg":
            print("Using ddpg agent to predict attacker!")
            state_dim = self.part_nets_per_round * 3
            action_dim = self.part_nets_per_round
            self.ddpg_agent = DDPG_Agent(state_dim=state_dim, action_dim=action_dim, device=self.device)
    ...
    def run(self, wandb_ins=None):
        ...
        for flr in range(1, self.fl_round + 1):
            ...
            if flr in self.attacking_fl_rounds:
                ...
                selected_node_indices = np.random.choice(
                    self.num_nets, size=self.part_nets_per_round - 1, replace=False
                )
                num_data_points = [
                    len(self.net_dataidx_map[i]) for i in selected_node_indices
                ]
                total_num_dps_per_round = (
                    sum(num_data_points) + self.num_dps_poisoned_dataset
                )

                logger.info(
                    "FL round: {}, total num data points: {}, num dps poisoned: {}".format(
                        flr, num_data_points, self.num_dps_poisoned_dataset
                    )
                )

                net_freq = [self.num_dps_poisoned_dataset / total_num_dps_per_round] + [
                    num_data_points[i] / total_num_dps_per_round
                    for i in range(self.part_nets_per_round - 1)
                ]
                logger.info(
                    "Net freq: {}, FL round: {} with adversary".format(net_freq, flr)
                )
                # pdb.set_trace()

                # we need to reconstruct the net list at the beginning
                net_list = [
                    copy.deepcopy(self.net_avg) for _ in range(self.part_nets_per_round)
                ]
                ...
                for net_idx, net in enumerate(net_list):
                    ...
                    if net_idx == 0:
                        global_user_idx = (
                            -1
                        )  # we assign "-1" as the indices of the attacker in global user indices
                        pass
                    else:
                        ...
                        if self.attack_case == "edge-case":
                            train_dl_local, _ = get_dataloader(
                                self.dataset,
                                "./data",
                                self.batch_size,
                            ...
                            train_dl_local, _ = get_dataloader_normal_case(
                                self.dataset,
                                "./data",
                                self.batch_size,
                                self.test_batch_size,
                    ...
                    if net_idx == 0:
                        logger.info(
                            "@@@@@@@@ Working on client: {}, which is Attacker".format(
                                net_idx
                            )
                        )
                    else:
                        logger.info(
                            "@@@@@@@@ Working on client: {}, which is Global user: {}".format(
                                net_idx, global_user_idx
                            )
                        )
                    ...
                    optimizer = optim.SGD(
                        net.parameters(),
                        lr=self.args_lr * self.args_gamma ** (flr - 1),
                        momentum=0.9,
                        weight_decay=1e-4,
                    )  # epoch, net, train_loader, optimizer, criterion
                    adv_optimizer = optim.SGD(
                        net.parameters(),
                        lr=self.adv_lr * self.args_gamma ** (flr - 1),
                        momentum=0.9,
                        weight_decay=1e-4,
                    )  # looks like adversary needs same lr to hide with others
                    prox_optimizer = optim.SGD(
                        wg_clone.parameters(),
                        lr=self.args_lr * self.args_gamma ** (flr - 1),
                        momentum=0.9,
                        weight_decay=1e-4,
                    ...
                    for param_group in optimizer.param_groups:
                        logger.info(
                            "Effective lr in FL round: {} is {}".format(
                                flr, param_group["lr"]
                            )
                        )
                    ...
                    if net_idx == 0:
                        ...
                        if self.model_replacement:
                            ...
                            logger.info(
                                "Attacker before scaling : Norm = {}".format(
                                    torch.norm(v)
                                )
                            )
                            ...
                            v = torch.nn.utils.parameters_to_vector(net.parameters())
                            logger.info(
                                "Attacker after scaling : Norm = {}".format(
                                    torch.norm(v)
                                )
                            )
                        ...
                        adv_norm_diff = calc_norm_diff(
                            gs_model=net,
                            vanilla_model=self.net_avg,
                            epoch=e,
                            fl_round=flr,
                            mode="bad",
                        )
                        ...
                        # we can print the norm diff out for debugging
                        honest_norm_diff = calc_norm_diff(
                            gs_model=net,
                            vanilla_model=self.net_avg,
                            epoch=e,
                            fl_round=flr,
                ...
                selected_node_indices = np.random.choice(
                    self.num_nets, size=self.part_nets_per_round, replace=False
                )
                num_data_points = [
                    len(self.net_dataidx_map[i]) for i in selected_node_indices
                ]
                ...
                logger.info(
                    "Net freq: {}, FL round: {} without adversary".format(net_freq, flr)
                )
            ...
            if self.defense_technique == "no-defense":
                ...
                logger.info(
                    "#### Let's Look at the Nom Diff Collector : {} ....; Mean: {}".format(
                        norm_diff_collector, np.mean(norm_diff_collector)
                    )
                )
                ...
                for net_idx, net in enumerate(net_list):
                    self._defender.exec(
                        client_model=net, global_model=self.net_avg,
                    )
            elif self.defense_technique == "krum":
                net_list, net_freq = self._defender.exec(
                    client_models=net_list,
                ...
                pseudo_avg_net = fed_avg_aggregator(
                    net_list, net_freq, device=self.device, model=self.model
                )
                net_list, net_freq, pred_g_attacker = self._defender.exec(
                    client_models=net_list,
                    num_dps=[self.num_dps_poisoned_dataset] + num_data_points,
                    net_freq=net_freq,
                    net_avg=self.net_avg,
                ...
                print(
                    "Predicted Attackers in FL iteration-{}: {}".format(
                        flr, pred_g_attacker
                    )
                )            
                    
            elif self.defense_technique == "ddpg":
                pseudo_avg_net = fed_avg_aggregator(
                    net_list, net_freq, device=self.device, model=self.model
                )
                # if flr > 1:
                #     prev_reward = 1.0 #need to be updated
                term1, term2, term3 = 0.0, 0.0, 0.0
                if flr > 1:
                    prev_reward, term1, term2, term3 = get_ddqg_reward(aggregated_grad, vectorize_nets, krum_w, self.prev_avg, delta_acc, flr)
                # else:
                self.prev_avg = flatten_model(self.net_avg)  
                
                net_list, net_freq, aggregated_grad, vectorize_nets = self.ddpg_agent.get_action(
                    client_models=net_list,
                    pseudo_avg=pseudo_avg_net,
                    num_dps=[self.num_dps_poisoned_dataset] + num_data_points,
                    net_avg=self.net_avg,
                    net_freq=net_freq,
                    g_user_indices=g_user_indices,
                ...
                krum_w = get_krum_vectors(client_models=net_list,
                    # pseudo_avg=pseudo_avg_net,
                    num_dps=[self.num_dps_poisoned_dataset] + num_data_points,
                    num_workers=10,
                    num_adv=1
                ...
                flatten_local_models = [
                    flatten_model(net_list[net_idx]).cpu().data.numpy()
                    for net_idx in range(len(net_list))
                ]
                updates = (
                    np.asarray(flatten_local_models)
                    - self.flatten_net_avg.cpu().data.numpy()
                )
                ...
                net_list, net_freq, cs, eu_dist = self._defender.exec(
                    client_models=net_list,
                    num_dps=[self.num_dps_poisoned_dataset] + num_data_points,
            ...
            self.net_avg = fed_avg_aggregator(
                net_list, net_freq, device=self.device, model=self.model
            )
            ...
            calc_norm_diff(
                gs_model=self.net_avg,
                vanilla_model=self.vanilla_model,
                epoch=0,
                fl_round=flr,
                mode="avg",
            )
            ...
            logger.info(
                "Measuring the accuracy of the averaged global model, FL round: {} ...".format(
                    flr
                )
            )

            overall_acc, raw_acc = test(
                self.net_avg,
                self.device,
                self.vanilla_emnist_test_loader,
                test_batch_size=self.test_batch_size,
            ...
            if(flr >= 2):
                delta_acc = overall_acc - prev_acc
            prev_acc = overall_acc
            backdoor_acc, _ = test(
                self.net_avg,
                self.device,
                self.targetted_task_test_loader,
        ...
        results_filename = get_results_filename(
            self.poison_type,
            self.attack_method,
            self.model_replacement,
            self.project_frequency,
            self.defense_technique,
            self.norm_bound,
            self.prox_attack,
            False,
            self.model,
...
class FixedPoolFederatedLearningTrainer(FederatedLearningTrainer):
    def __init__(self, arguments=None, *args, **kwargs):
        ...
        self.reputation_score = [
            1.0 for _ in range(arguments["num_nets"])
        ]  # init reputation score for all clients in the FL systems
        self.local_update_history = [
            [0.0] for _ in range(arguments["num_nets"])
        ]  # theta i,t => keep track of update history of clients
        ...
        if arguments["defense_technique"] == "no-defense":
            ...
            self._defender = KmeansBased(
                num_workers=self.part_nets_per_round, num_adv=1
            )
        elif arguments["defense_technique"] == "krum-multilayer":
            self._defender = KrMLRFL(
                num_workers=self.part_nets_per_round, num_adv=1, num_valid=1
        ...
        self.__attacker_pool = np.random.choice(
            self.num_nets, int(self.num_nets * self.attacker_percent), replace=False
        )
    ...
    def run(self, wandb_ins=None):
        ...
        for flr in range(1, self.fl_round + 1):
            ...
            if self.defense_technique == "contra":
                ...
                selected_node_indices = np.random.choice(
                    self.num_nets, size=self.part_nets_per_round, replace=False, p=probs
                )
            ...
            selected_attackers = [
                idx for idx in selected_node_indices if idx in self.__attacker_pool
            ]
            selected_honest_users = [
                idx for idx in selected_node_indices if idx not in self.__attacker_pool
            ]
            logger.info(
                "Selected Attackers in FL iteration-{}: {}".format(
                    flr, selected_attackers
                )
            )
            num_data_points = []
            ...
            for net_idx, global_user_idx in enumerate(selected_node_indices):
                ...
                if global_user_idx in selected_attackers:
                    ...
                    if self.model_replacement:
                        v = torch.nn.utils.parameters_to_vector(net.parameters())
                        logger.info(
                            "Attacker before scaling : Norm = {}".format(torch.norm(v))
                        )
                        # adv_norm_diff = calc_norm_diff(gs_model=net, vanilla_model=self.net_avg, epoch=e, fl_round=flr, mode="bad")
                        # logger.info("||w_bad - w_avg|| before scaling = {}".format(adv_norm_diff))
                        ...
                        v = torch.nn.utils.parameters_to_vector(net.parameters())
                        logger.info(
                            "Attacker after scaling : Norm = {}".format(torch.norm(v))
                        )
            ...
            for net_idx, global_client_indx in enumerate(selected_node_indices):
                ...
                updates = (
                    flatten_local_model.cpu().data.numpy()
                    - self.flatten_net_avg.cpu().data.numpy()
                )
                # print(updates)
                # local_updates = np.asarray(flatten_local_model.cpu().data.numpy() - self.flatten_net_avg.cpu().data.numpy())
                self.local_update_history[global_client_indx] = (
                    self.local_update_history[global_client_indx] + updates
                    if self.local_update_history[global_client_indx] is not None
                    else updates
                )
            ...
            if self.defense_technique == "no-defense":
                ...
                thr = 0.5
                k = 3
                net_list, net_freq, repu_s = self._defender.exec(
                    client_models=net_list,
                    net_freq=net_freq,
                    selected_node_indices=selected_node_indices,
                    historical_local_updates=self.local_update_history,
                    reputations=self.reputation_score,
                    delta=delta,
                    threshold=thr,
                    k=k,
                )
                ...
                print("selected_attackers: ", selected_attackers)
            ...
            adv_norm_diff = (
                1.0 * sum(current_adv_norm_diff_list) / len(current_adv_norm_diff_list)
                if len(current_adv_norm_diff_list)
                else 0
            )
